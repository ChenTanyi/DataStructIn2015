...
void readln(vector<int>& v){
	char ch;
	while (!check(ch = std :: cin.get()));
	v.push_back(ch);
	while (check(ch = std :: cin.get())) v.push_back(ch);
	std :: reverse(v.begin(), v.end());
	while (v.size() && *v.rbegin() == 0) v.pop_back();
}
...
bool check(vector<int> & a, vector<int> & b){
	...
	if (a.size() < b.size())
		return 0;
	for (vector<int>::reverse_iterator ait = a.rbegin(), bit = b.rbegin() ; ait != a.rend() ; ++ ait, ++ bit){
		if (*ait > *bit)
			return 1;
		if (*ait < *bit)
			return 0;
...
void subtract(vector<int> & a, vector<int> & b){
	int i, sub = 0;
	for (i = 0 ; i < static_cast<int>(a.size()) && i < static_cast<int>(b.size()) ; ++ i){
		a[i] -= sub + b[i];
		if (a[i] < 0){
			a[i] += 10;
			sub = 1;
...
int main(){
	...
	if (check(a,b)){
		...
		if (static_cast<int>(a.size()) == 0) a.push_back(0);
		...
		if (static_cast<int>(b.size()) == 0) b.push_back(0);
