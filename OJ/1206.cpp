...
bool check(char * a , const char * b , int size){
	for (int i = 0 ; i < size ; ++ i){
		if (a[i] != b[i])
			return 0;
	}
...
//freopen("a.in","r",stdin);
	while (std :: cin >> buffer){
		switch (buffer[0]){
			case 'b':
				if (check(buffer , "begin" , 5))
					s.push('b');
				break;
			case 'i':
				if (check(buffer , "if" , 2))
					s.push('i');
				break;
			case 't':
				if (check(buffer , "then" , 4)){
