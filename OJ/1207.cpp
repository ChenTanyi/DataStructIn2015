...
//freopen("debug.in","r",stdin);
	...
	for (int i = 0 ; i < n ; ++ i){
		...
		while (t > time){
			int num = 0;
			while (num < std :: max(8 , 10 - vh.size()) && !vk.empty()){
				int x = vk.back();vk.pop_back();
				++ num;
				tk += time - x;
			}
			while (num < 10 && !vh.empty()){
				int x = vh.back();vh.pop_back();
				++ num;
				th += time - x;
			}
			time += 10;
	...
	//std :: cout << vk.size() << " " << vh.size();
	int headk = 0 , headh = 0;
	while (headk < vk.size() || headh < vh.size()){
		int counter = 0;
		...
		while (headh < vh.size() && vh[headh] <= time && counter < 10){
            int x = vh[headh ++];
            th += time - x;
            ++ counter;
		}
