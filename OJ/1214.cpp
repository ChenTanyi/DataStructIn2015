...
public:
    ...
    BinaryTreeNode(T val = T(), BinaryTreeNode* p = 0, BinaryTreeNode* l = 0, BinaryTreeNode* r = 0):
        value(val),parent(p),left(l),right(r){}
};

template <typename T>
bool operator == (const BinaryTreeNode<T> & a, const BinaryTreeNode<T> & b){
    return a.value == b.value;
}

vector<BinaryTreeNode<int> > v;
vector<BinaryTreeNode<int>*>t;
...
void levelorder(BinaryTreeNode<T>* node){
    ...
    for (int head = 0 ; head < t.size() ; ++ head){
        BinaryTreeNode<T>* x = t[head];
        if (flag) printf(" %d",x -> value);
        else {
            flag = 1;
            printf("%d",x -> value);
        }
